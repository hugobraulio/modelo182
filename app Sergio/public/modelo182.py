...
def reg_tipo1(df, p):
    ...
    linea1  = p["TIPO_REGISTRO"] + p["MODELO_DECLARACION"] + p["EJERCICIO"] 
    linea1 += p["NIF_DECLARANTE"]
    linea1 += '{0:<40}'.format(p["DENOMINACION_DECLARANTE"])
    linea1 += p["TIPO_SOPORTE"] + '{0:<9}'.format(p["TELEFONO_CONTACTO"]) 
    linea1 += '{0:<40}'.format(p["PERSONA_CONTACTO"])
    linea1 += '{0:<13}'.format(p["NUMERO_JUSTIFICANTE_DECLARACION"]) 
    linea1 += p["DECLARACION_COMPLEMENTARIA_O_SUSTITUTIVA"]
    linea1 += p["ID_DECLARACION_ANTERIOR"] + NUM_TOTAL_REGISTROS_DECLARADOS
    linea1 += '{0:013.0f}'.format(IMPORTE_DONATIVOS) 
    linea1 += '{0:.2f}'.format(IMPORTE_DONATIVOS)[-2:]
    linea1 += p["NATURALEZA_DECLARANTE"] + p["NIF_TITULAR_PATRIMONIO"] 
    linea1 += p["APELLIDOS_NOMBRE_TITULAR_PATRIMONIO"]
